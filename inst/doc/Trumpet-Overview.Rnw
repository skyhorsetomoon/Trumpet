...
# Collect the path of files of all the aligned MeRIP-seq data files in BAM format.
...
# Group the files according to their experimental conditions and sample type.
...
# We use GTF in the following example
...
After preparing the sequencing data and annotation inforamtion, we can call the main function to generate the assessment report. 
