get_readscount2 <- function(IP_BAM, Input_BAM, contrast_IP_BAM, contrast_Input_BAM, 
                            condition1, condition2, GENE_ANNO_GTF = NA, GENOME = NA, UCSC_TABLE_NAME = "knownGene", 
                            TXDB = NA, sample_size = NA, output_dir = NA) {
  ...
  if (is.na(output_dir)) {
    output_dir <- getwd()
    save(result, IP_BAM, Input_BAM, contrast_IP_BAM, contrast_Input_BAM, 
         condition1, condition2, file = paste(output_dir, "Input_Infor.Rdata", 
                                              sep = "/"))
  }
  if (!is.na(output_dir)) {
    save(result, IP_BAM, Input_BAM, contrast_IP_BAM, contrast_Input_BAM, 
