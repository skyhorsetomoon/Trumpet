...
C-test is used to compare two poisson means in IP and Input samples for peak calling in exomePeak R package. `ctest_evluate` can calculate the percentages of enrichment signal using c-test method at different fold-change in IP samples compared with unified Input samples and calculate the percentages of enrichment signal in unified IP samples compared with different Input samples. It will plot some figures to show the assessment result and identify problem biological replicates.
