...
The MeRIP-seq data for quality assessment should include IP(the methylated RNA immunoprecipitation samples) biological replicates and Input(RNA-seq data samples) control biological replicates in one or two conditions, which means that you can evaluate the IP, Input samples or IP, Input and contrast IP, contrast Input samples. These samples should be BAM format  with your alignment reads. The reads count and other useful parameters have been gained from the function of **get\_readscount2** firstly. User should load the **Input_Infor.Rdata** that they need to assess.
...
In this module, we can get the reads alignment summary information and check the percentage of partial reads count of every sample in in each transcript and each bin. This metric can detect the sequencing depth and show the state of RNA transcription abundance. The result can return six tables.
...
* **file** : The path of input BAM file 

* **sample_name** : The name of input BAM files to evaluate

**2. Table Read Summary Information**

* **total_reads** : Total number of alignment reads in each BAM file

* **exon_reads** : The number of reads mapping to exon region in each BAM file

* **intron_reads** : The number of reads mapping to intron region in each BAM file

* **percent_intron** : The percentage of reads count in intron was calculated by dividing the sum reads count of exon and intron.

* **UTR5_reads** :  The number of reads mapping to UTR5 region in each BAM file

* **CDS_reads** : The number of reads mapping to CDS region in each BAM file

* **UTR3_reads** : The number of reads mapping to UTR3 region in each BAM file
...
**Note:The x-axis denotes the percentages of bins in the whole genome and the y-axis denotes percentages of the cumulative reads count after normalization. The black line divides the genome into two regions: the background region and the enrichment signal region. The point at which the distance between the IP and Input percentages is maximized shows the difference between IP and Input; the greater the separation between IP and Input at this point, the better the IP enrichment at appropriate sequencing depth, which the parameter `Scale_factor` can stand for**

## 4.2 Evaluate enrichment signal strength using c-test method

C-test is used to compare two poisson means in IP and Input samples for peak calling in exomePeak R package. `ctest_evluate` can calculate the percentage of enrichment signal using c-test method at different fold-change in IP samples compared with unified Input samples and calculate the percentages of enrichment signal in unified IP samples compared with different Input samples. It will plot beautiful pictures to show the assessment result and identify problem biological replicates.
...
In order to identify the difference within one group of  the IP biological replicates and the Input biological replicates, we can use the hierachical clustering method after reads count having been normalized between samples.
...
In this work, the observed SD is tightly relate to the mean in each condition. The relationship should be similarity within biological replicates in one condition. Using this strategy, we can identify some abnormal samples. The value of mean and SD have been taken the logarithm.
...
```{r eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,fig.width=10,fig.height=14}
enrichmentdist <- .SES_enrich_DF(result, IP_BAM, Input_BAM, contrast_IP_BAM, contrast_Input_BAM)
```

**Note:The percentage of enrichment region of our known data is in the range of 15% to 20%. The percentage of signal reads count of our knowing data is in the range of 90% to 95%. The Scale_factor of our known data is in the range of 0.05 to 0.15 and 0.2 to 0.25. If the user's data has the high overlap to our knowing data, the enrihcment degree of user's degree is normal.**
